...
JControls.AnimationBox = Class.create(JControls.Object, {
    ...
    showing:function ($super, x, y, w, h) {
        if (this.animationData&&this.imageData) {
            let w1 = this.imageData.cellSize.width;
            let h1 = this.imageData.cellSize.height;
            let x1 = (this.animationData.beginPoint.WNum + this.animationOffset.WNum + this.animationPlayNum) % this.imageData.picNum.WNum;
            let y1 = this.animationData.beginPoint.HNum + this.animationOffset.HNum + parseInt((this.animationData.beginPoint.WNum
...
JControls.Label = Class.create(JControls.Object, {//从父类继承
    ...
    fontSize:10,//字体大小
    isSelect:false,
    initialize: function ($super, argP, argWH,argtext) {//覆盖父类中构造函数
        $super(argP, argWH);//执行父类中构造函数
        this.text = argtext;
...
JControls.MessageBox = Class.create(JControls.Object, {
    initialize:function ($super,  argWH, argAString,argP) {
        ...
        let messageTitle = new JControls.Label({x:0, y:0}, "系统提示");
        ...
        for (let i = 0; i < argAString.length; i++) {//添加消息内容
            let m = new JControls.Label({x:0, y:h}, argAString[i]);
...
module.exports = JControls;
// window.JControls = JControls;